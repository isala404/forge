...
Install `@forgelib/openfeature-provider` with the official `@openfeature/server-sdk` to evaluate Forge's typed flags through OpenFeature. The provider preserves stable variants and reasons, installs no global hooks, and exposes the official OpenTelemetry event hook for application-scoped registration. Startup code can use `configGetMany` and `flagDetailsMany` for ordered 256-item reads. `configSnapshot` returns a frozen, expiring view containing only requested keys; declare `no_secrets` or protect the encoded snapshot outside the trusted server boundary.
