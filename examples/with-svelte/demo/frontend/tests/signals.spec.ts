...
// The signals client SDK posts every signal kind to the unified `/_api/signal`
// endpoint with a `{type, payload}` discriminator. Each test below proves that
// the browser SDK reaches that endpoint with the shape the server accepts, and
// that the five signal kinds from the goal — view, event, identify, error
// report, and correlation_id — all flow end-to-end.
...
test.describe("signals: client SDK end-to-end", () => {
  ...
  test("identify: SDK emits an identify event with user_id + traits", async ({
    ...
    await page.evaluate((uid) => {
      window.forgeSignals.identify(uid, {
        plan: "enterprise",
  ...
  test("report: captureError sends stack, context, breadcrumbs, page_url", async ({
    page,
  }) => {
    ...
    await page.evaluate((m) => {
      ...
      window.forgeSignals.captureError(new Error(`${m} manual capture`), {
        feature: "tests",
      });
    }, marker);
