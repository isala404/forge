...
// The app only subscribes to the todos query once authenticated (the
// `listTodos$()` store is created inside the `{:else}` of the auth gate), so
// reactivity readiness can't be detected until after sign-up. Arm the
// subscribe wait before submitting the form, then await it once the authed
// view renders.
...
test("two users cannot see each other's todos", async ({ browser }) => {
  ...
  // Each user gets an isolated context (isolated storage = isolated token).
  ...
  // Bob signs up in a brand new context.
  ...
  // Bob must not see Alice's todo and vice versa.
