...
- **Background workers** run in-process: `fanout` (mark delivered + bump unread kv, idempotent on message id), `reap` (hard-delete disappearing messages + their blob), `fail` (always nacks → exercises `fail.dlq`), and a scheduler tick that fires due `schedule().at` jobs.
...
Brings up Postgres 18, the backend (which compiles the `forgelib` native addon from source; the repo-root `.dockerignore` strips committed `*.node` binaries), and the shared React SPA pointed at the backend. SPA at http://127.0.0.1:5173, backend at http://127.0.0.1:8082/graphql.
