...
Read `app/routes.py` first for the FastAPI handlers, then `app/main.py` for startup. Forge calls stay inline in the route handlers; `app/types.py` is only Pydantic shapes, and `app/utils.py` is only small validation/key helpers.
