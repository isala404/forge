...
A compact map of the surface. The method names line up across languages: Rust groups
them under an accessor and takes option structs, while Node and Python put each one
on the client with plain arguments. Where a binary-safe variant exists (for example
`kvGetBytes`), it's noted but not given its own row.
...
| Managed worker | `worker(name)` | (loop) | `typed.run_worker` |
...
- **Decision** (rate limit): `allowed`, `limit`, `remaining`, `resetAfter`, `retryAfter`.
