...
If you used `forge new`, you already have a `frontend/` directory with the FORGE runtime pre-configured.

**Scaffolding options:**
- `forge new my-app` - Full demo frontend with User CRUD UI
- `forge new my-app --empty` - Minimal frontend with starter page
- `forge new my-app --minimal` - No frontend (backend only)

If creating a frontend manually:
...
├── types.ts       # Generated types from Rust
└── index.ts       # Re-exports
```
...
│   │   │   ├── TaskList.svelte
│   │   │   └── TaskCard.svelte
│   │   └── auth.ts          # Auth store (if needed)
│   ├── routes/
│   │   ├── +layout.svelte   # Provider setup
...
Then restart your TypeScript server (Cmd+Shift+P → "TypeScript: Restart TS Server").
