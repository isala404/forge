...
```
  ┌────────────────┐         ┌────────────────┐
  │   Your App     │         │  FORGE Server  │
  │                │         │                │
  │ subscribe()────┼────────►│                │
  │                │         │  PostgreSQL    │
  │ ◄──WebSocket───┼─────────┤  NOTIFY/LISTEN │
  │                │         │                │
  │  UI updates!   │         │                │
  └────────────────┘         └────────────────┘
```
...
Use `reset()` to clear the subscription state (useful for logout or route changes):

```svelte
<script lang="ts">
  import { subscribe } from '$lib/forge';

  const userProfile = subscribe('get_my_profile', {});
...
Use `createConnectionStore()` for reactive connection status:

```svelte
<script lang="ts">
  import { createConnectionStore } from '$lib/forge';

  // Reactive store that tracks connection state
...
The connection store provides:

```typescript
interface ConnectionStatusStore {
  connected: boolean;      // WebSocket is connected
  reconnecting: boolean;   // Currently attempting reconnection
