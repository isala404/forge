...
Tests that require database access. Use `TestDatabase` with **explicit** configuration:
...
`TestDatabase` provides PostgreSQL access for integration tests with **explicit configuration**. This prevents accidental use of production databases.
...
// Option 2: From TEST_DATABASE_URL environment variable
// Note: Uses TEST_DATABASE_URL (not DATABASE_URL) for safety
let db = TestDatabase::from_env().await?;

// Option 3: Embedded PostgreSQL (requires embedded-test-db feature)
#[cfg(feature = "embedded-test-db")]
let db = TestDatabase::embedded().await?;
```

**Important:** The `.env` file `DATABASE_URL` is NEVER automatically read. You must explicitly choose one of the options above.

### Isolated Databases

For tests that modify data, create an isolated database:

```rust
let db = TestDatabase::from_env().await?;
let isolated = db.isolated("my_test").await?;

isolated.execute("CREATE TABLE users (id UUID PRIMARY KEY)").await?;

// Use isolated.pool() for queries
...
```bash
# With external database (explicit env var)
...
| `claim(key)` | Get a custom claim value |
