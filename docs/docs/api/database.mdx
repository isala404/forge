...
FORGE creates essential indexes automatically, including GIN indexes on all JSONB columns in system tables for efficient querying:

```sql
-- GIN indexes on JSONB columns (created automatically)
CREATE INDEX idx_forge_jobs_input_gin ON forge_jobs USING GIN (input);
CREATE INDEX idx_forge_metrics_labels_gin ON forge_metrics USING GIN (labels);
CREATE INDEX idx_forge_logs_fields_gin ON forge_logs USING GIN (fields);
CREATE INDEX idx_forge_traces_attributes_gin ON forge_traces USING GIN (attributes);
-- ... and more
```
...
```sql
-- Find jobs with specific input fields
SELECT * FROM forge_jobs WHERE input @> '{"user_id": "123"}';

-- Find metrics with matching labels
SELECT * FROM forge_metrics WHERE labels @> '{"service": "api"}';

-- Search logs by fields
SELECT * FROM forge_logs WHERE fields @> '{"request_id": "abc"}';
```
...
-- GIN index for your JSONB columns
