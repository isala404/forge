...
Zero to running app in five commands:
...
Requires Rust, Docker (with Compose v2), and Bun. `forge migrate up` starts a local Postgres container and applies migrations. `forge test` runs your backend unit tests and the Playwright suite against a live stack.

The rest of this page walks through building the app from scratch.
...
Run `forge doctor` at any time to verify your environment is set up correctly:

```bash
forge doctor
```

It checks Rust toolchain version, `cargo-sqlx`, Docker, `bun`/`dx`, `DATABASE_URL` reachability, and `.sqlx/` cache freshness — printing `[OK]`, `[!]`, or `[X]` for each item with a remediation hint on failure.
...
The `#[forge::query]` macro generates an `inventory::submit!` call automatically, so no manual registration is needed.
...
Your `src/main.rs` uses `auto_register()`, which picks up all annotated handlers automatically:
