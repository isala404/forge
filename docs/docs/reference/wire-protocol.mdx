...
| `POST` | `/_api/rpc` | Batch call (multiple functions) |
| `GET` | `/_api/events` | Open an SSE stream |
| `POST` | `/_api/subscribe` | Subscribe to a query |
| `POST` | `/_api/unsubscribe` | Unsubscribe from a query |
| `POST` | `/_api/subscribe/job/{job_id}` | Subscribe to job status |
| `POST` | `/_api/subscribe/workflow/{workflow_id}` | Subscribe to workflow status |
...
{
  "requests": [
    { "function": "get_users", "args": { "team_id": "abc" } },
...
```http
POST /_api/subscribe/job/550e8400-...
Content-Type: application/json
...
```http
POST /_api/subscribe/workflow/550e8400-...
Content-Type: application/json
