...
| `context` | `async fn context(&self) -> Value` | Get persisted job context data |
| `set_context` | `async fn set_context(&self, context: Value) -> Result<()>` | Replace persisted job context |
| `update_context` | `async fn update_context(&self, key: &str, value: Value) -> Result<()>` | Update a single context key |
