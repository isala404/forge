...
| `POST` | `/_api/admin/sessions/{session_id}/revoke` | Revoke a session's cached auth and evict its SSE connection |
...
Returns `503 reactor_unavailable` if the reactor isn't running on this node (e.g. headless migrate-only invocations).
