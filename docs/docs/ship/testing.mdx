...
Each test context builder creates an isolated environment with mocked authentication, configurable environment variables, HTTP interception, and dispatch recording. Tests run against real PostgreSQL when needed via embedded or isolated databases. This avoids global state pollution between tests.
...
For embedded database tests, use embedded PostgreSQL. Requires the `embedded-db` feature.

```toml
# Cargo.toml
[dev-dependencies]
forge = { version = "0.1.0", features = ["embedded-db"] }
```
