...
| `refresh_cookie`      | bool     | `true`           | Browser clients keep the refresh token in an `HttpOnly; Secure; SameSite=Strict` cookie. See [SSE & refresh-token security](./security#sse-authentication). |
...
| `rate_limit_per_minute`| u32      | `600`   | Per-IP cap on `/signal` requests in a 60s window |
