...
:::note Calling a webhook from the browser
If your own frontend triggers a webhook (rather than an external service), the browser must send `X-Webhook-Timestamp` alongside the signature header — omitting it returns 401. The gateway's CORS policy already allows both headers, so a same-config `fetch` works once the timestamp is attached.
:::
