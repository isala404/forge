...
1. One node claims the run via `INSERT ... ON CONFLICT DO UPDATE` (reclaiming stale runs if the previous executor timed out)
2. The database's unique constraint on `(cron_name, scheduled_time)` ensures only one node wins
...
| `timezone` | `"tz"` | `"UTC"` | IANA timezone (e.g., `"America/New_York"`, `"Europe/London"`). Validated at compile time; an unknown tz fails with `Invalid timezone: "X". Must be an IANA tz database name (e.g., "UTC", "America/New_York").` |
...
| `group` | `"string"` | `"default"` | Sharded leader election group (accepted by the macro but not yet active — all cron tasks run under a single scheduler leader regardless of this value) |
...
| `ctx.delay()` | `chrono::Duration` | Time between scheduled and actual execution |
...
| `ctx.trace_id()` | `String` | Current trace identifier |
| `ctx.span()` | `&Span` | Current tracing span |
...
SET id = EXCLUDED.id, status = 'running', node_id = EXCLUDED.node_id,
    started_at = NOW(), completed_at = NULL, error = NULL
WHERE forge_cron_runs.status = 'running'
  AND forge_cron_runs.started_at < NOW() - $5
```

Multiple nodes can attempt the same insert. PostgreSQL's unique constraint ensures exactly one succeeds. The node that wins (`rows_affected > 0`) executes the cron. If a previous run is stuck (status still `running` past the stale threshold, default 15 minutes), another node can reclaim it.
...
mod tests {
    ...
    async fn test_logging() {
        ...
        ctx.log.info("Started", json!({}));
        ctx.log.warn("Warning", json!({"count": 42}));
        ctx.log.error("Failed", json!({"reason": "timeout"}));

        // CronLog writes to tracing; use tracing-test or tracing-subscriber
        // to capture and assert log output in tests
    }
}
```
