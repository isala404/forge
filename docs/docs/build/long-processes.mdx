...
Forge derives a blake3 (128-bit, truncated) signature from each workflow definition's persisted contract: the name, version, step keys, wait keys, timeout, and input/output types. This signature is recorded when a workflow run starts and checked on every resume.
...
Within five seconds, the next `/_api/ready` probe re-runs the drain check and flips back to `200`. To mark a run as an operator-initiated cancellation rather than a plain failure, use the force-abort admin endpoint instead. There is no admin HTTP route for bulk drain; the database is the operator interface.
