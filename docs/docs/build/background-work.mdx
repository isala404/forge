...
| `compensate` | `"handler"` | none | Cancellation compensation handler function name |
...
Jobs automatically set `owner_subject` from `ctx.auth.principal_id()` on dispatch.

```rust
#[forge::mutation(transactional)]
pub async fn start_export(ctx: &MutationContext, input: ExportInput) -> Result<Uuid> {
    // owner_subject is set automatically from the authenticated principal
    let job_id = ctx.dispatch_job("export_data", json!({
        "format": input.format,
    })).await?;
...
// Query a user's own jobs
#[forge::query]
pub async fn my_jobs(ctx: &QueryContext) -> Result<Vec<JobSummary>> {
    let subject = ctx.auth.require_subject()?;

    sqlx::query_as("SELECT id, job_type, status FROM forge_jobs WHERE owner_subject = $1 ORDER BY created_at DESC")
        .bind(subject)
        .fetch_all(ctx.db())
        .await
        .map_err(Into::into)
...
The `owner_subject` column is indexed for efficient per-user queries.
...
pub async fn create_order(ctx: &MutationContext, order: NewOrder) -> Result<Order> {
    let order = ctx.db().fetch_one(
        sqlx::query_as("INSERT INTO orders ... RETURNING *")
            .bind(&order)
    ).await?;
...
| `ctx.heartbeat()` | `async Result<()>` | Send heartbeat to prevent stale reclaim |
| `ctx.save(key, value)` | `async Result<()>` | Save data for retries/compensation |
| `ctx.saved()` | `async Value` | Get all saved data |
| `ctx.set_saved(data)` | `async Result<()>` | Replace all saved data |
| `ctx.is_cancel_requested()` | `async Result<bool>` | Check if cancellation requested |
| `ctx.check_cancelled()` | `async Result<()>` | Return error if cancelled |
...
Every job moves through a defined set of statuses stored in `forge_jobs.status`:
...
Claimed
  │
  ▼ executor picks up
Running ──────────────────── cancel requested ──► CancelRequested
  │                                                      │
  │                                                      ▼
  ├── success ──────────────────────────────────► Completed
  │
  ├── error, attempts remaining ────────────────► Pending (rescheduled)
  │
  ├── error, attempts exhausted ────────────────► DeadLetter
  │
  └── cancelled ────────────────────────────────► Cancelled
```
...
| Status | Description |
|--------|-------------|
| `Pending` | Waiting to be claimed. Initial state, and state after a failed attempt with retries remaining. |
| `Claimed` | A worker has locked the record but not yet started execution. Brief transitional state. |
| `Running` | Handler is executing. Heartbeat timer active. |
| `CancelRequested` | Cancellation was requested while the job is running. Handler can detect via `ctx.check_cancelled()`. |
| `Completed` | Handler returned `Ok`. Final state. |
| `Failed` | Handler returned `Err` and `max_attempts` was exhausted. Final state. |
| `DeadLetter` | Alias for permanently failed jobs moved out of the active queue. Final state. |
| `Cancelled` | Job was cancelled. Compensation handler runs if configured. Final state. |

`Claimed` jobs that never transition to `Running` within 5 minutes are released back to `Pending` by stale recovery. This handles crashed workers between the claim and the spawn.
...
A portion of permits (default: 4 of 8) is reserved for system jobs — internal workflow resumptions and cron dispatches. User job floods cannot exhaust those permits, so workflow and cron execution remains responsive under load.

### Stale Job Recovery

A background task runs every 60 seconds. Claimed jobs that haven't started within 5 minutes and running jobs with no recent heartbeat are released back to pending:
...
Crashed workers' jobs are reclaimed automatically. The runtime sends automatic heartbeats every 30 seconds for running jobs. Long-running jobs can also call `ctx.heartbeat()` explicitly to update the heartbeat timestamp.
...
If found, the existing job ID returns without creating a duplicate. The `idempotency_key` column is indexed for fast lookups.
