...
pub async fn charge_card(ctx: &JobContext, args: ChargeArgs) -> Result<Receipt> {
    ctx.update_context("charge_id", serde_json::json!(args.charge_id)).await?;
...
pub async fn rollback_charge(
    ...
    ctx.update_context("cancel_reason", serde_json::json!(reason)).await?;
    refund(&args.charge_id).await
}
```
