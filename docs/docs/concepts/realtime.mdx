...
FORGE watches PostgreSQL changes
        │
        ▼
When data changes → Query re-runs → UI updates
```
...
│                         PostgreSQL Database                              │
│                                                                          │
│  INSERT/UPDATE/DELETE ──► Trigger ──► NOTIFY 'forge_changes'            │
│                          (forge_notify_change)                          │
└───────────────────────────────────┬─────────────────────────────────────┘
                                    │
                                    ▼
...
│                         ChangeListener                                 │
│                                                                        │
│  • PostgreSQL PgListener on 'forge_changes' channel                   │
│  • Parses notification payload: "table:operation:row_id:columns"      │
│  • Converts to Change struct and broadcasts to Reactor                │
└───────────────────────────────────┬───────────────────────────────────┘
                                    │
                                    ▼
┌───────────────────────────────────────────────────────────────────────┐
...
│  • Finds affected subscriptions via SubscriptionManager               │
│  • Debounces rapid changes (50ms window, 200ms max wait)              │
│  • Coalesces multiple changes to same table                           │
│  • Returns list of SubscriptionIds to invalidate                      │
└───────────────────────────────────┬───────────────────────────────────┘
                                    │
                                    ▼
┌───────────────────────────────────────────────────────────────────────┐
...
│  • Re-executes queries for invalidated subscriptions                  │
│  • Compares result hash to detect actual data changes                 │
│  • Pushes updates only when data differs                              │
└───────────────────────────────────┬───────────────────────────────────┘
                                    │
                                    ▼
┌───────────────────────────────────────────────────────────────────────┐
...
│  • Routes data updates to correct sessions                           │
...
|------|-------------|--------|
| `None` | No tracking (disabled) | Implemented |
| `Table` | Invalidate when any row in the table changes | Implemented |
| `Row` | Invalidate only when specific read rows change | Implemented |
| `Adaptive` | Auto-select based on subscription count | Implemented |

The `AdaptiveTracker` automatically switches between row-level and table-level tracking based on subscription patterns.

#### Adaptive Tracking

The `AdaptiveTracker` optimizes invalidation overhead by dynamically adjusting tracking granularity:

```rust
pub struct AdaptiveTrackingConfig {
    /// Switch to table tracking when subscriptions exceed this (default: 100)
    pub row_threshold: usize,
    /// Switch to row tracking when subscriptions drop below this (default: 50)
    pub table_threshold: usize,
    /// Maximum rows to track per table (default: 10,000)
    pub max_tracked_rows: usize,
    /// Re-evaluation interval (default: 60s)
...
1. **Few subscriptions (< 50):** Row-level tracking
   - Only rows actually read trigger invalidation
   - Most efficient when subscriptions are sparse

2. **Many subscriptions (> 100):** Table-level tracking
   - Any change to the table triggers invalidation
   - Avoids overhead of tracking thousands of individual rows

3. **Hysteresis (50-100):** Keep current mode
   - Prevents oscillation when subscription count fluctuates

```rust
// AdaptiveTracker decides invalidation scope
let tracker = AdaptiveTracker::new(config);

// Record new subscription with specific rows
tracker.record_subscription("users", Some(vec!["user-1", "user-2"])).await;

// Check if a change should trigger invalidation
if tracker.should_invalidate("users", "user-1").await {
    // Invalidate subscriptions watching this row
}

// Get current tracking mode
let mode = tracker.get_mode("users").await;
// Returns: TrackingMode::Row or TrackingMode::Table
```
...
- `list_tasks` → tracks `tasks` table
- `find_projects` → tracks `projects` table
- `fetch_orders` → tracks `orders` table
...
**What is implemented:**
- Full ChangeListener → InvalidationEngine → Reactor → WebSocket pipeline
