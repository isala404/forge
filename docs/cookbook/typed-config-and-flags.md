...
- **v1 is boolean-only.** `FlagRule` is `#[non_exhaustive]`; non-boolean OpenFeature variants and `EvalCtx.attributes`-based targeting are post-v1 (the field exists but v1 rules read only `targeting_key`).
