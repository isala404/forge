...
Implemented fluent workflow step API with ctx.step().
- Added StepRunner in `crates/forge-core/src/workflow/step_runner.rs` with fluent builder pattern
- Added ctx.step(name, fn) method to WorkflowContext returning StepRunner
- StepRunner supports: .timeout(), .compensate(), .optional(), .run()
- Automatic step resume on workflow restart (checks is_step_completed)
- Compensation handlers registered and run in reverse order on failure (saga pattern)
- Updated scaffolded workflow template to demonstrate both APIs:
  - Fluent API: ctx.step("name", || async {...}).timeout(...).compensate(...).run().await
  - Low-level API: ctx.is_step_completed/record_step_start/record_step_complete for manual control
- Clear comments explaining when to use each API

Cleaned up scaffolded project templates.
- Removed cleanup_inactive_users_cron (unnecessary example)
- Added "Auto-generated by FORGE - DO NOT EDIT" comments to all runtime files:
  - runtime/types.ts, client.ts, context.ts, stores.ts, api.ts, index.ts
  - runtime/ForgeProvider.svelte (HTML comment format)

Simplified scaffolded app comments and merged migrations.
- Merged 0001_create_users.sql and 0002_create_app_stats.sql into single 0001_initial.sql
- Removed verbose doc comments from Rust templates (job, cron, workflow, users, app_stats)
- Removed section banner comments from main.rs, +page.svelte templates
- Simplified runtime_generator.rs: removed JSDoc from stores.ts (job/workflow trackers)
- Simplified new.rs frontend templates: removed JSDoc from types.ts, api.ts, index.ts
- Code now self-documenting, comments only where explaining "why" not "what"

Updated all dependencies to latest versions.
- toml 0.8 -> 0.9, tonic 0.12 -> 0.14, prost 0.13 -> 0.14
- sysinfo 0.32 -> 0.37, darling 0.20 -> 0.23
- console 0.15 -> 0.16, dialoguer 0.11 -> 0.12, indicatif 0.17 -> 0.18
- jsonwebtoken 9 -> 10 (using rust_crypto backend, updated auth.rs for new API)
- tokio 1.42 -> 1.48, hyper 1.5 -> 1.8, uuid 1.11 -> 1.19
- regex 1.11 -> 1.12, regex-lite 0.1 -> 0.1.8
- Removed outdated home pin from scaffolded app template

Implemented up/down migration system with CLI commands.
- Extended Migration struct with `up_sql` and `down_sql` fields in `crates/forge-runtime/src/migrations/runner.rs`
- Added `Migration::parse()` method to split on `-- @down` marker
- Updated `forge_migrations` table with `down_sql TEXT` column
- Added `MigrationRunner::rollback()` method to execute down SQL in reverse order
- Added `MigrationRunner::status()` returning `MigrationStatus` with applied/pending lists
