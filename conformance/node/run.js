'use strict'
// Cross-language conformance runner — Node side. Reads ../scenarios/*.json and
// runs each scenario against the forge-node binding on a throwaway database.
// Asserts the observed failure set equals exactly the `node` entries in
// ../known_gaps.json. See ../README.md.
//
...
// ---- error code mapping (binding spells codes UPPER_SNAKE; canonical is Pascal) ----
const CODE_MAP = {
  CONFIG: 'Config',
...
async function withTestDb(fn) {
  ...
  await admin.end()
  try {
    return await fn(swapDb(ADMIN_URL, name))
  } finally {
...
// ---- op dispatch: canonical op + args -> binding call -> normalized result ----
async function dispatch(client, op, args) {
  switch (op) {
    ...
    case 'ratelimit.check': {
      const d = await client.rateLimitCheck(args.bucket, args.key, args.max, args.per_seconds, args.fail_open ?? null)
      return {
        allowed: d.allowed,
        limit: d.limit ?? null,
    ...
    case 'schedule.at':
      return await client.scheduleAt(args.when_epoch_ms, args.queue, valueToString(args.payload))
    case 'schedule.cron':
...
async function runScenario(scenario) {
  return withTestDb(async (url) => {
    ...
    for (let i = 0; i < scenario.steps.length; i++) {
      ...
      if (!clients.has(ns)) clients.set(ns, await ForgeClient.connectWith(url, { kvNamespace: ns }))
      const client = clients.get(ns)
      const args = resolve(step.args ?? {}, captures)
      let outcome
      try {
        outcome = { ok: true, value: await dispatch(client, step.op, args) }
      } catch (e) {
