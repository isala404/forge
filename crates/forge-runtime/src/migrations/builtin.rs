...
pub fn get_builtin_migrations() -> Vec<Migration> {
    vec![Migration::new("0000_forge_internal_v1", FORGE_INTERNAL_V1)]
}

/// Version 1 of FORGE internal schema.
///
/// Creates all core tables needed for the runtime:
...
CREATE TABLE IF NOT EXISTS forge_workflow_steps (
    id UUID PRIMARY KEY,
    run_id UUID NOT NULL REFERENCES forge_workflow_runs(id) ON DELETE CASCADE,
...
mod tests {
    ...
    fn test_get_builtin_migrations() {
        ...
        assert_eq!(migrations[0].name, "0000_forge_internal_v1");
    }
