...
fn render_struct(table: &TableDef) -> String {
    ...
    // ForgeUpload doesn't impl PartialEq or Serialize/Deserialize, so upload
    // fields are skipped from the wire payload. Mutation routing handles them
    // out-of-band via multipart.
    ...
    for field in &table.fields {
        if contains_upload(&field.rust_type) {
            output.push_str("    #[serde(skip)]\n");
        }
        output.push_str(&format!(
            "    pub {}: {},\n",
            field.name,
