...
impl PgQueue {
    ...
    async fn try_claim(&self, queue: &str, vis_secs: f64) -> Result<Option<Job>> {
        ...
        Ok(row.map(|r| Job {
            ...
            attempt: u32::try_from(r.attempts).unwrap_or(0).saturating_add(1),
            max_attempts: u32::try_from(r.max_attempts).unwrap_or(0),
            leased_until: to_system_time(r.leased_until),
...
impl Queue for PgQueue {
    async fn enqueue(&self, queue: &str, payload: Bytes, opts: EnqueueOpts) -> Result<JobId> {
        ...
        let backoff = Json(opts.backoff.clone());
        ...
        obs::instrument("queue", "enqueue", span, async move {
            ...
            let Some(dedup_id) = opts.dedup_id.clone() else {
                let id = sqlx::query_scalar!(
                    r#"INSERT INTO forge_jobs (queue, payload, status, attempts, max_attempts, backoff, available_at)
                       VALUES ($1, $2, 'available', 0, $3, $4, now() + make_interval(secs => $5))
                       RETURNING id"#,
                    queue,
                    payload_vec,
                    max_attempts,
            ...
            let claimed = sqlx::query_scalar!(
                r#"INSERT INTO forge_job_dedup (queue, dedup_id, job_id, expires_at)
                   ...
                   ON CONFLICT (queue, dedup_id) DO UPDATE
                     ...
                     WHERE forge_job_dedup.expires_at <= now()
                   RETURNING job_id"#,
                queue,
                dedup_id,
                new_id,
            ...
            match claimed {
                Some(_) => {
                    sqlx::query!(
                        r#"INSERT INTO forge_jobs (id, queue, payload, status, attempts, max_attempts, backoff, available_at)
                           VALUES ($1, $2, $3, 'available', 0, $4, $5, now() + make_interval(secs => $6))"#,
                        new_id,
                        queue,
                        payload_vec,
                        max_attempts,
                ...
                None => {
                    ...
                    let existing = sqlx::query_scalar!(
                        "SELECT job_id FROM forge_job_dedup WHERE queue = $1 AND dedup_id = $2",
                        queue,
                        dedup_id,
                    )
                    .fetch_one(&mut *tx)
        ...
        obs::instrument("queue", "nack", span, async move {
            ...
            let row = sqlx::query!(
                r#"SELECT attempts AS "attempts!", max_attempts AS "max_attempts!",
                          backoff AS "backoff!: Json<Backoff>"
                   FROM forge_jobs
                   WHERE id = $1 AND lease_token = $2 AND status = 'leased'
                   FOR UPDATE"#,
                id,
            ...
            let delay = opts
                .retry_in
                .unwrap_or_else(|| row.backoff.0.delay_for_attempt(new_attempts, seed));
...
/// Derive a stable per-job jitter seed from the job id's first 8 bytes.
fn seed_from_id(id: Uuid) -> u64 {
    let b = id.as_bytes();
    let mut buf = [0u8; 8];
    if let Some(prefix) = b.get(..8) {
        buf.copy_from_slice(prefix);
    }
    u64::from_le_bytes(buf)
}

/// Postgres `TIMESTAMPTZ` (chrono) to `SystemTime`. Lease deadlines are always
/// in the future, so the non-negative branch is the live path.
