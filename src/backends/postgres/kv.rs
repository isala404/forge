...
use crate::core::kv::{Kv, MAX_KEY_BYTES, MAX_VALUE_BYTES, SetMode, SetOpts};
...
impl PgKv {
    ...
    /// Strip the namespace prefix from a stored key before returning it.
    ...
    /// Delete expired rows to reclaim space (reads already hide them). Returns
...
/// Resolve `SetOpts.ttl` into the optional seconds value the SQL takes.
fn opt_ttl_secs(ttl: Option<Duration>) -> Result<Option<f64>> {
    match ttl {
        None => Ok(None),
        Some(d) => Ok(Some(ttl_to_secs(d)?)),
    }
}
    ...
    async fn scan(
        ...
        obs::instrument("kv", "scan", span, async move {
            ...
            let next = if (rows.len() as i64) < limit_i {
                ...
                rows.last().map(|k| Cursor::new(k.clone()))
            };
